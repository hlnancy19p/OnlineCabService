...
Create an order<br>
  Passenger: <select name="applier" id="applier">
	...
	<span id="app_msg" style="color: #ff0000"></span>
	<br>
	...
	<form>
		<fieldset>
			...
			<br /> Order ID : ${orderInfo['id']}<br /> <input id="oid"
				...
				style="display: none"> <br /> Passenger: <select
				name="applier" id="applier">
				<c:forEach items="${appliersList}" var="applier" varStatus="al">
					<c:if test="${orderInfo['applier'] !=  applier['fullname']}">
						<option value="${applier['id']}">${applier['companynumber']} ${applier['fullname']}</option>
					</c:if>
					<c:if test="${orderInfo['applier'] ==  applier['fullname']}">
						<option value="${applier['id']}" selected="selected">${applier['companynumber']} ${applier['fullname']}</option>
					</c:if>
				</c:forEach>
			</select><span id="app_msg" style="color: #ff0000"></span><br> pickup
			Time: <input id="pt" type="text" name="pickupTime"
			...
			<br> <input id="status" type="text" name="statusId"
				value="${orderInfo['statusCode']}" style="display: none"> <input
				id="version" type="text" name="version"
				value="${orderInfo['version']}" style="display: none"> <input
				id="creater" type="text" name="creater"
				value="${orderInfo['createrId']}" style="display: none"> <a

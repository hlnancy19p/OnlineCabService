...
function editPageDialog(orderId){
	...
	var url="/OnlineCabBooking/passenger/editOrder.htm";
	$.ajax({
		url : url,
		type : "POST",
		async : false,
...
function createOrder(){
	...
	var url="/OnlineCabBooking/passenger/createOrder.htm";
	$.ajax({
		url : url,
		type : "POST",
		async : false,
		...
		success : function(data) {
			...
			if(message != null){
				...
				if(receiveMessage.indexOf("Success")>0){
					...
					setTimeout("top.location.href='passenger/viewOrders.htm'", 1); 
				}
				else{
					alert("create Fail!");
				}
...
function updateOrder(){
	...
	// use ajax to send params to back end and receive result
	var url="/OnlineCabBooking/passenger/updateOrder.htm";
	$.ajax({
		url : url,
		type : "POST",
		async : false,
